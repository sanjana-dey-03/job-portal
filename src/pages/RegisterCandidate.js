...
const RegisterCandidate = ({ onClose, onOpenLogin }) => {
    ...
    const handleRegister = async () => {
        ...
        try {
            // Check if email or phone already exists in either 'candidates' or 'employers'
            const collectionsToCheck = ["candidates", "employers"];
            for (const colName of collectionsToCheck) {
                const qEmail = query(collection(db, colName), where("email", "==", email));
                const qPhone = query(collection(db, colName), where("phone", "==", phone));
